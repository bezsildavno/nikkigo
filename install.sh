...
printf '%sВНИМАНИЕ: изменение программного обеспечения роутера выполняется на ваш страх и риск.%s\n' "$C_YELLOW" "$C_RESET"
printf '%sNikkiGo предоставляется «как есть», без гарантий.%s\n' "$C_YELLOW" "$C_RESET"
say "Перед продолжением убедитесь, что у вас есть резервная копия и доступ к роутеру."
read_input "Для подтверждения ответственности введите 322"
if [ "$REPLY" != '322' ]; then
	say "Операция отменена. Изменения не внесены."
	exit 0
fi
...
say ""
say "============================================================"
say " NikkiGo — установка Taproom Nikki на OpenWrt"
say "============================================================"
say ""
