...
Использование проекта регулируется [отказом от ответственности](DISCLAIMER.md).
...
4. определяет, установлен ли Taproom Nikki;
...
Перед ежедневным обновлением NikkiGo временно сохраняет текущий выбор всех
selector-групп. После проверки нового профиля сначала назначаются безопасные
значения новым или изменившимся группам, затем допустимые прежние значения
восстанавливаются. Удалённая или переименованная группа не прерывает обновление.
Если восстановленный выбор не обеспечивает доступ в интернет, recovery
проверяет альтернативы по одной и откатывает каждую неудачную попытку.
Постоянное хранение выбора Mihomo включается через
`nikki.mixin.selection_cache=1`.

SSH-пароль не сохраняется и не передаётся установщику: его запрашивает
системный SSH-клиент.

При ошибке установщик показывает этап и диагностику. Обратитесь к вашему провайдеру подписки. В обращение нужно приложить
